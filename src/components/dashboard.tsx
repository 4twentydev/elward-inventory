...
export function Dashboard({ onNavigate }: DashboardProps) {
	...
	const categoryConfig: Record<ItemCategory, { icon: string; color: string }> =
		{
			...
			<div className="max-w-7xl mx-auto p-6 space-y-6">
				...
				<div className="bg-gradient-to-r from-blue-900/20 to-purple-900/20 border border-blue-500/30 rounded-xl p-6">
					<div className="flex items-center gap-3 mb-4">
						...
						<div>
							<h3 className="text-lg font-bold text-white">
								Quick Transfer & Return
							</h3>
							<p className="text-sm text-slate-400">
								Fast 1-2 touch inventory movements
							</p>
						</div>
					</div>
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
						<div className="bg-slate-800/50 rounded-lg p-4">
							<div className="flex items-center gap-3 mb-2">
								...
								<h4 className="font-semibold text-blue-400">Quick Transfer</h4>
							</div>
							<p className="text-sm text-slate-300 mb-3">
								Instantly move materials from stock to CNC, ELU, or Shipping
						...
						<div className="bg-slate-800/50 rounded-lg p-4">
							<div className="flex items-center gap-3 mb-2">
								<span className="text-2xl">↶</span>
								<h4 className="font-semibold text-emerald-400">Quick Return</h4>
							</div>
							<p className="text-sm text-slate-300 mb-3">
								Easily return unused materials from CNC, ELU, or Shipping back
								to stock
							</p>
							<div className="flex flex-wrap gap-2">
								<span className="px-2 py-1 bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 rounded text-xs">
									← CNC
								</span>
								<span className="px-2 py-1 bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 rounded text-xs">
									← ELU
					...
					<button
						type="button"
						onClick={() => onNavigate("inventory")}
						className="w-full px-4 py-3 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white rounded-lg font-semibold transition-colors"
					>
						Go to Inventory to Use Quick Transfer →
					</button>
				</div>
							...
							<p className="text-2xl font-bold text-rose-500">
								{
									recentTransactions.filter(
										(t) => t.type === "pull" || t.type === "adjustment",
									).length
								}
							</p>
